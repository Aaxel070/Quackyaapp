...
public class MainActivity extends Activity {
    ...
    private void seleccionarAnimal(String id, LinearLayout cardNueva) {
        if (cardAnimalActiva != null && cardAnimalActiva != cardNueva) {
            ...
            if (cardAnimalActiva.getChildCount() >= 3)
                actualizarBadge((TextView) cardAnimalActiva.getChildAt(2), false, true);
        }
        aplicarFondoCard(cardNueva, true, true);
        if (cardNueva.getChildCount() >= 3)
            actualizarBadge((TextView) cardNueva.getChildAt(2), true, true);
    ...
    private void alPresionarActivar() {
        boolean ok = verificarPermiso("Mostrar sobre apps") && verificarPermiso("Micrófono")
            && verificarPermiso("Contactos") && verificarPermiso("Teléfono");
    ...
    // ─────────────────────────────────────────────────────────────────────────
    //  INICIAR SERVICIO
    //  ✅ FIX: Detiene el servicio anterior antes de arrancar uno nuevo.
    //  Así el servicio SIEMPRE lee el animal y voz más recientes al arrancar.
    // ─────────────────────────────────────────────────────────────────────────
    private void iniciarServicio() {
        Toast.makeText(this, "Cargando " + emojiAnimalActual() + "...", Toast.LENGTH_SHORT).show();
 
        // 1. Detener el servicio que ya está corriendo (si hay uno)
        stopService(new Intent(this, DuckOverlayService.class));
 
        // 2. Esperar 600ms para que se detenga completamente, luego arrancar uno nuevo
        new Handler().postDelayed(() -> {
            ...
            // 3. Mandar la app al fondo para que el animal sea visible
            moveTaskToBack(true);
        }, 600);
    }
    ...
    public void onRequestPermissionsResult(int req, String[] perms, int[] results) {
        ...
        if (req == PERM_REQUEST) {
            ...
            if (verificarPermiso("Mostrar sobre apps") && verificarPermiso("Micrófono") && verificarPermiso("Contactos") && verificarPermiso("Teléfono"))
                iniciarServicio();
